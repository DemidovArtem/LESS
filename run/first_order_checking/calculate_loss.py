...
parser.add_argument("--train_batch", type=str, default="train")
...
if args.info_type == "grads" and args.gradient_type == "adam":
    if args.optimizer_state_path is not None:
        model_path = args.optimizer_state_path
    else:
...
if args.task is not None:
    ...
    train_batch = torch.load(args.train_batch)
    dataset_id = train_batch["dataset_id"][-128:]
    data_id = train_batch["data_id"][-128:]

    dataset_names = [train_file_names[ii] for ii in dataset_id]
    iid = [f"{d}_{idd}" for d, idd in zip(dataset_names, data_id)]
    print(iid[:10])

    dataset = dataset.filter(lambda x: x["id"] in iid)
